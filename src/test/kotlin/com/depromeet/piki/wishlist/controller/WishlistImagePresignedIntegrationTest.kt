...
class WishlistImagePresignedIntegrationTest : IntegrationTestSupport() {
    ...
    fun `presigned 발급하면 요청한 개수만큼 items_raw key 와 uploadUrl 을 받는다`() {
        ...
        insertMember(userId)
        try {
            val body = objectMapper.writeValueAsString(presignImages(listOf("image/png", "image/jpeg"), contentLength = 2_048L))
    ...
    @Test
    fun `contentLength 가 없으면 UPLOAD-004 로 400 이 반환된다`() {
        val mockMvc = buildMockMvc()
        val userId = UUID.randomUUID()
        insertMember(userId)
        try {
