...
class StubImageStorage : ImageStorage {
    ...
    val defaultPresignBehavior: (String, String, Long, Duration) -> String = { key, _, _, _ -> "$BASE_URL/$key?X-Amz-Signature=stub" }
    var presignBehavior: (String, String, Long, Duration) -> String = defaultPresignBehavior

    // 서명에 묶인 바이트 수 기록 — 발급 검증이 통과시킨 contentLength 가 그대로 서명으로 넘어가는지 본다.
