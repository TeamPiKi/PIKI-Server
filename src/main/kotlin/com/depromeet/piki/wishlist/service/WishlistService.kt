    ...
    fun getWishlist(
        ...
        val entries =
            pageWishes.map { wish ->
                ...
                WishWithItem(wish = wish, item = item, snapshot = displayById[pointer.getId()] ?: pointer).withSourcePlatform()
            }
    ...
    fun recoverWishItem(
        ...
        if (listOfNotNull(name, price, currency, image).isEmpty()) {
            memo?.let {
                ...
                return result.copy(snapshot = itemDisplayService.resolveDisplay(result.snapshot)).withSourcePlatform()
            }
        }
