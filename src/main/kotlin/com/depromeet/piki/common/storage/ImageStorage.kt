...
interface ImageStorage {
    ...
    // 서버가 key·contentType(·contentLength) 을 서명에 박으므로, 클라이언트는 그 값으로만 올릴 수 있다(S3 가 강제).
    // contentLength 가 null 이면 크기는 서명에 넣지 않는다. SDK 예외는 ImageStorageException(502)으로 변환한다.
    fun presignUpload(
        key: String,
        contentType: String,
        contentLength: Long?,
